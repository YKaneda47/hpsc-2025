...
int main(int argc, char** argv) {
  ...
  for(int irank=0; irank<size; irank++) {
    ...
    //MPI_Recv(jbody, N/size, MPI_BODY, recv_from, 0, MPI_COMM_WORLD, MPI_STATUS_IGNOR\
E);                                                                                    
    MPI_Win win;
    MPI_Win_create(jbody, N/size*sizeof(int), sizeof(int), MPI_INFO_NULL, MPI_COMM_WOR\
LD, &win);
    MPI_Win_fence(0, win);
